 ...
 * Liquid splash burst at cursor with curling ripples and waves (WebGL fluid simulation).
 */
...
export interface SplashCursorProps {
 ...
 TRANSPARENT?: boolean;
 /** Optional className for the canvas (e.g. for z-index or overlay). */
 className?: string;
...
function pointerPrototype(): Pointer {
 ...
 down: false,
 moved: false,
 color: { r: 0, g: 0, b: 0 } as SplashCursorColor
};
}
...
export function SplashCursor({
 ...
 if (!extVal.supportLinearFiltering) {
 ...
 /** WebGL1 OES_texture_half_float exposes HALF_FLOAT_OES; not in TS lib. */
 ...
 `,
 extVal.supportLinearFiltering ? null : ['MANUAL_FILTERING']
 );
 ...
 const texType = extVal.halfFloatTexType;
 const rgba = extVal.formatRGBA;
 const rg = extVal.formatRG;
 const r = extVal.formatR;
 const filtering = extVal.supportLinearFiltering ? gl.LINEAR : gl.NEAREST;
 gl.disable(gl.BLEND);
 ...
 if (!extVal.supportLinearFiltering && advectionProgram.uniforms.dyeTexelSize) {
 ...
 function splat(x: number, y: number, dx: number, dy: number, color: SplashCursorColor) {
 splatProgram.bind();
 ...
 function updatePointerMoveData(pointer: Pointer, posX: number, posY: number, color: SplashCursorColor) {
 pointer.prevTexcoordX = pointer.texcoordX;
 pointer.prevTexcoordY = pointer.texcoordY;
 pointer.texcoordX = posX / canvasEl.width;
 pointer.texcoordY = 1 - posY / canvasEl.height;
 ...
 const onMouseDown = (e: MouseEvent) => {
  ...
  updatePointerDownData(mainPointer, -1, posX, posY);
  clickSplat(mainPointer);
 };
 ...
 function handleFirstMouseMove(e: MouseEvent) {
  ...
  const color = generateColor();
  updateFrame();
  updatePointerMoveData(mainPointer, posX, posY, color);
 ...
 const onMouseMove = (e: MouseEvent) => {
  const posX = scaleByPixelRatio(e.clientX);
  const posY = scaleByPixelRatio(e.clientY);
  updatePointerMoveData(mainPointer, posX, posY, mainPointer.color);
 };
 ...
 function handleFirstTouchStart(e: TouchEvent) {
  ...
  for (let i = 0; i < touches.length; i++) {
   ...
   updateFrame();
   updatePointerDownData(mainPointer, touch.identifier, posX, posY);
  }
