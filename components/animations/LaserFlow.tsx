 ...
 * Dynamic laser light that flows onto a surface
 * 
 * Based on React Bits Laser Flow component
...
#define DITHER_STRENGTH 1.0
 ...
 v+=amp*vnoise(p); p=m*p*2.0; amp*=0.5;
 }
 return v;
 }
 float rGate(float x,float w){return smoothstep(-EPS,w,x)*smoothstep(w*2.0+EPS,w+EPS,x);}
 float vWisps(vec2 uv,float topF){
 float yf=uv.y*W_CELL+uFlowTime*uWSpeed,sp=tri01(yf*0.5),ep=tri01(yf*0.5+0.5);
 float sum=0.0,xS=sign(uv.x),yPix=uv.y;
 float bGain=pow(1.0-smoothstep(-W_CURVE_RANGE,0.0,yPix),W_BOTTOM_EXP);
 for(int sgn=-1;sgn<=1;sgn+=2){
 float sgnF=float(sgn);
 for(int i=0;i<W_LANES;++i){
 if(i>=W_LANES) break;
 float off=W_BASE_X+float(i)*W_LAYER_GAP,xc=sgnF*(off*xS);
 float dx=abs(uv.x-xc),lat=1.0-smoothstep(W_HALF,W_HALF+W_AA,dx),amp=exp(-off*W_SIDE_DECAY);
 float seed=h21(vec2(off,sgnF*17.0)),yf2=yf+seed*7.0,ci=floor(yf2),fy=fract(yf2);
 float seg=mix(W_SEG_MIN,W_SEG_MAX,h21(vec2(ci,off*2.3)));
 float spR=h21(vec2(ci,off+sgnF*31.0)),seg1=rGate(fy,seg)*step(spR,sp);
 if(ep>0.0){float spR2=h21(vec2(ci*3.1+7.0,off*5.3+sgnF*13.0)); float f2=fract(fy+0.5); seg1+=rGate(f2,seg*0.9)*step(spR2,ep);}
 sum+=amp*lat*seg1;
 }
 }
 float span=smoothstep(-3.0,0.0,yPix)*(1.0-smoothstep(R_V-6.0,R_V,yPix));
...
export function LaserFlow({
  ...
  return <div ref={mountRef} className={cn('absolute inset-0 pointer-events-none', className)} style={style} />;
}
